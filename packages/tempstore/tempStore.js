...
function mountStorage() {
  ...
  if (Package["chriscinelli:gridfs"] && (Package["chriscinelli:worker"] || !Package["chriscinelli:filesystem"])) {
    ...
    // Use the gridfs
    FS.TempStore.Storage = new FS.Store.GridFS('_tempstore', { internal: true });
  } else if (Package["chriscinelli:filesystem"]) {

    // use the Filesystem
    FS.TempStore.Storage = new FS.Store.FileSystem('_tempstore', { internal: true });
  } else {
    throw new Error('FS.TempStore.Storage is not set: Install chriscinelli:filesystem or chriscinelli:gridfs or set it manually');
  }
